...
class JUZT_DEPLOY_BASIC_OAuth_Service
{
    /**
    ...
    {
        $this->session_token = get_option('JUZT_DEPLOY_BASIC_oauth_token');
        $this->refresh_token = get_option('JUZT_DEPLOY_BASIC_refresh_token');

        if (!wp_next_scheduled('JUZT_DEPLOY_BASIC_auto_refresh_token')) {
            wp_schedule_event(time(), 'hourly', 'JUZT_DEPLOY_BASIC_auto_refresh_token');
        }

        // Hook del cron
        add_action('JUZT_DEPLOY_BASIC_auto_refresh_token', array($this, 'auto_refresh_token_if_needed'));
    }
    ...
    {
        $return_url = urlencode(admin_url('admin.php?page=juzt-deploy-basic&tab=settings'));
    ...
    public function refresh_access_token()
    {
        ...
        if ($code === 200 && isset($data_response['access_token'])) {
            ...
            update_option('JUZT_DEPLOY_BASIC_oauth_token', $data_response['access_token']);
            update_option('JUZT_DEPLOY_BASIC_refresh_token', $data_response['refresh_token']);
    ...
    public function handle_oauth_callback()
    {
        ...
        update_option('JUZT_DEPLOY_BASIC_oauth_token', $access_token);
    ...
    {
        $last_refresh = get_option('JUZT_DEPLOY_BASIC_token_last_refresh', 0);
