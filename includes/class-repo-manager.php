...
class JUZT_DEPLOY_BASIC_Repo_Manager
{
    ...
    public function __construct()
    {
        ...
        require_once JUZT_DEPLOY_BASIC_PLUGIN_DIR . 'includes/class-git-cli.php';
        require_once JUZT_DEPLOY_BASIC_PLUGIN_DIR . 'includes/class-git-api.php';
        add_filter('JUZT_DEPLOY_BASIC_test', array($this, 'test_filters'), 10, 2);
        add_filter('JUZT_DEPLOY_BASIC_queue_commit', array($this, 'handle_queue_commit_filter'), 10, 4);
    }
    ...
    {
        error_log('🔄 JUZT_DEPLOY_BASIC_queue_commit called - Stack: ' . wp_debug_backtrace_summary());
        $allow_auto_commit = get_option('JUZT_DEPLOY_BASIC_auto_commit', 'no') === 'yes';
    ...
    public function detect_git_mode()
    {
        ...
        if (!function_exists('exec')) {
            update_option('JUZT_DEPLOY_BASIC_git_mode', 'api');
            update_option('JUZT_DEPLOY_BASIC_git_mode_reason', 'exec() function disabled');
            return 'api';
        }
        ...
        if (in_array('exec', $disabled)) {
            update_option('JUZT_DEPLOY_BASIC_git_mode', 'api');
            update_option('JUZT_DEPLOY_BASIC_git_mode_reason', 'exec() in disable_functions');
            return 'api';
        }
        ...
        if ($return_var === 0) {
            $mode = 'cli';
            update_option('JUZT_DEPLOY_BASIC_git_mode_reason', 'Git CLI available: ' . implode(' ', $output));
        } else {
            update_option('JUZT_DEPLOY_BASIC_git_mode_reason', 'Git command not found');
        }

        update_option('JUZT_DEPLOY_BASIC_git_mode', $mode);
        return $mode;
    }
    ...
    public function get_git_mode()
    {
        ...
        $detected_mode = get_option('JUZT_DEPLOY_BASIC_git_mode');
    ...
    private function get_git_instance()
    {
        ...
        if ($mode === 'cli') {
            return new JUZT_DEPLOY_BASIC_Git_CLI();
        } else {
            return new JUZT_DEPLOY_BASIC_Git_API();
        }
    }
    ...
    {
        require_once JUZT_DEPLOY_BASIC_PLUGIN_DIR . 'includes/class-progress-tracker.php';
        $progress = new JUZT_DEPLOY_BASIC_Progress_Tracker($job_id);
        ...
        if (!$git->is_available()) {
            ...
            return array(
                'success' => false,
                'error' => __('Git no está disponible en el sistema', 'juzt-deploy-basic')
            );
        }
        ...
        if (is_dir($destination)) {
            ...
            return array(
                'success' => false,
                'error' => sprintf(__('El directorio %s ya existe', 'juzt-deploy-basic'), $folder_handle)
            );
        }
        ...
        if (!$result['success']) {
            ...
            return array(
                'success' => false,
                'error' => __('Error al clonar repositorio: ', 'juzt-deploy-basic') . $result['error']
            );
        }
        ...
        return array(
            'success' => true,
            'message' => __('Repositorio clonado exitosamente', 'juzt-deploy-basic'),
            'path' => $destination,
    ...
    public function update_repository($identifier, $access_token = '', $job_id = null)
    {
        ...
        if (!$repo_info) {
            ...
            return array(
                'success' => false,
                'error' => __('No se encontró el repositorio', 'juzt-deploy-basic')
            );
        }
        ...
        if (!$result['success']) {
            ...
            return array(
                'success' => false,
                'error' => __('Error al actualizar repositorio: ', 'juzt-deploy-basic') . $result['error']
            );
        }
        ...
        return array(
            'success' => true,
            'message' => __('Repositorio actualizado exitosamente', 'juzt-deploy-basic'),
            'job_id' => $progress->get_job_id()
        );
    }
    ...
    public function switch_branch($identifier, $new_branch, $job_id = null)
    {
        ...
        $access_token = get_option('JUZT_DEPLOY_BASIC_oauth_token');
        ...
        if (!$result['success']) {
            ...
            return array(
                'success' => false,
                'error' => __('Error al cambiar rama: ', 'juzt-deploy-basic') . $result['error']
            );
        }
        ...
        return array(
            'success' => true,
            'message' => sprintf(__('Cambiado a rama %s exitosamente', 'juzt-deploy-basic'), $new_branch),
            'job_id' => $progress->get_job_id()
        );
    }
    ...
    public function remove_repository($identifier)
    {
        ...
        if (!$repo_info) {
            return array(
                'success' => false,
                'error' => __('No se encontró el repositorio en la base de datos', 'juzt-deploy-basic')
            );
        }
        ...
        if (is_dir($local_path)) {
            if (!$this->remove_directory($local_path)) {
                return array(
                    'success' => false,
                    'error' => __('Error al eliminar directorio físico', 'juzt-deploy-basic')
                );
            }
        }
        ...
        if ($deleted === false) {
            return array(
                'success' => false,
                'error' => __('Error al eliminar registro de base de datos', 'juzt-deploy-basic')
            );
        }

        return array(
            'success' => true,
            'message' => __('Repositorio eliminado exitosamente', 'juzt-deploy-basic')
        );
    }
    ...
    public function create_wp_content_zip($zip_name)
    {
        ...
        if (empty($zip_name)) {
            return array('success' => false, 'error' => 'Nombre de archivo inválido');
        }

        // Aumentar límites (Subimos a 1GB por si acaso, aunque ZipArchive usa menos)
        @set_time_limit(600);
        ...
        $zip_filepath = $temp_dir . $zip_name . '.zip';
        if (file_exists($zip_filepath)) {
            @unlink($zip_filepath);
        }

        // --- CAMBIO CLAVE: Usar ZipArchive en lugar de PclZip ---
        $zip = new ZipArchive();
        if ($zip->open($zip_filepath, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== TRUE) {
            return array('success' => false, 'error' => 'No se pudo crear el archivo ZIP con ZipArchive');
        }

        $wp_content_path = realpath(WP_CONTENT_DIR);
        $directories_to_include = array('themes', 'plugins', 'uploads', 'mu-plugins');

        foreach ($directories_to_include as $dir) {
            $full_path = $wp_content_path . DIRECTORY_SEPARATOR . $dir;
            ...
            if (is_dir($full_path) && is_readable($full_path)) {
                ...
                foreach ($files as $name => $file) {
                    if (!$file->isDir()) {
                        ...
                        // Creamos la ruta relativa dentro del ZIP (wp-content/directorio/archivo)
                        $relative_path = 'wp-content/' . $dir . '/' . substr($file_path, strlen($full_path) + 1);
                        $zip->addFile($file_path, $relative_path);
                    }
                }
                error_log("WPVTP: Directorio $dir añadido a la cola.");
            }
        }

        $zip->close(); // Aquí es donde se procesa y guarda realmente el archivo

        if (!file_exists($zip_filepath)) {
            return array('success' => false, 'error' => 'Error al generar el archivo final');
        }
    ...
    private function detect_modified_files($repo_path)
    {
        ...
        foreach ($iterator as $file) {
            if ($file->isFile()) {
                ...
                if ($relative_path === '.JUZT_DEPLOY_BASIC_metadata' || strpos($relative_path, '.') === 0) {
                    continue;
                }
    ...
    private function get_repo_token($repo_path)
    {
        ...
        if (!$metadata) {
            ...
            return get_option('JUZT_DEPLOY_BASIC_oauth_token');
        }
        ...
        $oauth_token = get_option('JUZT_DEPLOY_BASIC_oauth_token');
