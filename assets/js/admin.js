...
(function ($) {
  ...
  const DEBUG = typeof JUZT_DEPLOY_BASIC_ajax !== "undefined" && JUZT_DEPLOY_BASIC_ajax.debug === "1";
  ...
  function startProgressTracking(jobId, onComplete, onError, customContainer) {
    ...
    let pollInterval = setInterval(function () {
      $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
        action: "JUZT_DEPLOY_BASIC_get_progress",
        job_id: jobId,
        nonce: JUZT_DEPLOY_BASIC_ajax.nonce,
  ...
  $(document).ready(function () {
    ...
    $("#wpvtp-download-wp-content").on("click", function (e) {
      ...
      $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
        action: "JUZT_DEPLOY_BASIC_download_wp_content",
        zip_name: zipName,
        nonce: JUZT_DEPLOY_BASIC_ajax.nonce,
      })
        .done(function (response) {
    ...
    $(document).on("click", ".wpvtp-retry-commit", function () {
      ...
      $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
        action: "JUZT_DEPLOY_BASIC_retry_commit",
        commit_id: commitId,
        nonce: JUZT_DEPLOY_BASIC_ajax.nonce,
      })
        .done(function (response) {
    ...
    $(document).on("click", ".wpvtp-delete-commit", function () {
      ...
      $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
        action: "JUZT_DEPLOY_BASIC_delete_commit",
        commit_id: commitId,
        nonce: JUZT_DEPLOY_BASIC_ajax.nonce,
      }).done(function (response) {
        if (response.success) {
  ...
  function bulkDeleteSelectedCommits(){
    ...
    showConfirmModal(
      ...
      function () {
        ...
        jQuery.post(ajaxurl, {
          action: 'JUZT_DEPLOY_BASIC_bulk_delete_commits',
          nonce: JUZT_DEPLOY_BASIC_ajax.nonce,
  ...
  function searchRepositoriesByName(owner, term){
    jQuery.post(ajaxurl, {
      action: 'JUZT_DEPLOY_BASIC_search_repositories_by_name',
      nonce: JUZT_DEPLOY_BASIC_ajax.nonce,
      owner: owner,
      term: term,
    }, function (response) { 
  ...
  function initializeActions() {
    ...
    $(document).on("click", ".wpvtp-push-all-btn", function (e) {
      ...
      jQuery.post(ajaxurl, {
        action: 'JUZT_DEPLOY_BASIC_push_all_changes',
        nonce: JUZT_DEPLOY_BASIC_ajax.nonce,
  ...
  window.disconnectGitHub = function () {
    ...
    $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
      action: "JUZT_DEPLOY_BASIC_disconnect_github",
      nonce: JUZT_DEPLOY_BASIC_ajax.nonce,
    })
      .done(function (response) {
        if (response.success) {
  ...
  function loadOrganizations() {
    ...
    $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
      action: "JUZT_DEPLOY_BASIC_get_organizations",
      nonce: JUZT_DEPLOY_BASIC_ajax.nonce,
    })
      .done(function (response) {
  ...
  function loadRepositories(owner, type, installationId) {
    ...
    $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
      action: 'JUZT_DEPLOY_BASIC_get_repositories_by_installation',
      owner: owner,
      type: type,
  ...
  function loadBranches(owner, repo) {
    ...
    $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
      action: "JUZT_DEPLOY_BASIC_get_branches",
      owner: owner,
      repo: repo,
      nonce: JUZT_DEPLOY_BASIC_ajax.nonce,
    })
  ...
  function showInstallSummary() {
    ...
    const typeLabel =
      selectedRepo.detected_type === "theme" ? "🎨 Theme" : "🔌 Plugin";
  ...
  function installRepository() {
    ...
    $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
      action: "JUZT_DEPLOY_BASIC_clone_repository",
                ...
                .html(
                  `
                    ...
                    <p style="margin-top: 15px;">
                      <a href="${JUZT_DEPLOY_BASIC_ajax.admin_url}" class="button button-primary">View Repositories</a>
                    </p>
                  `
                )
                .show()
  ...
  function updateRepository(localPath, button) {
    ...
    $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
      action: "JUZT_DEPLOY_BASIC_update_repository",
  ...
  function removeRepository(localPath) {
    ...
    $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
      action: "JUZT_DEPLOY_BASIC_remove_repository",
  ...
  function switchBranch(localPath, newBranch) {
    ...
    // Buscar el botón que disparó esto para deshabilitarlo
    ...
    $.post(JUZT_DEPLOY_BASIC_ajax.ajax_url, {
      action: "JUZT_DEPLOY_BASIC_switch_branch",
      identifier: localPath, // Cambiar local_path por identifier
